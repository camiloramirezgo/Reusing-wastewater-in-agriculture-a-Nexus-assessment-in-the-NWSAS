...
class DataFrame:
    ...
    def calculate_population_water(self, region, urban_uni_water, rural_uni_water):
        ...
        self.df.loc[is_region & (1 - is_urban),
                    'PopulationWater'] = self.df.loc[is_region & (1 - is_urban), "PopulationFuture"] * rural_uni_water
        self.df.loc[is_region & is_urban,
    ...
    def calculate_lcow_capex(self, variable, investment_var, water_var,
        ...
        print(investment_var, self.df[investment_var].notna().sum())
    ...
    def calculate_reclaimed_water(self, pop_water_fraction, time=''):
        ...
        self.df[f'Irrigation{time}ReclaimedWater'] = self.df.set_index('Cluster').index.map(
            self.df.groupby('Cluster')[f'Irrigation{time}ReusedWater'].sum())
        self.df.loc[self.df[f'Irrigation{time}ReclaimedWater'] == 0,
                    f'Irrigation{time}ReclaimedWater'] = None
        print(f'Irrigation{time}ReclaimedWater: ', self.df[f'Irrigation{time}ReclaimedWater'].notna().sum())
        # print(self.df[f'Irrigation{time}ReclaimedWater'].notna().sum())
        # self.df.loc[self.df[f'Irrigation{time}ReclaimedWater'] == 0, f'Irrigation{time}ReclaimedWater'] = 0 # TODO: FIX!
    ...
    def get_storage(self, leakage, area_percent, storage_depth, agri_water_req, agri_non_recoverable, time=''):
        ...
        print(f'IrrigationWater{time}PerCluster: ', self.df[f'IrrigationWater{time}PerCluster'].notna().sum())
    ...
    def reused_water(self, pop_water_fraction, pop_percentage_of_reuse, time=''):
        ...
        """
        self.df[f'Irrigation{time}ReusedWater'] = self.df.filter(regex='stored_[1-9]').sum(axis=1)
        print(f'Irrigation{time}ReusedWater: ', self.df[f'Irrigation{time}ReusedWater'].notna().sum())
...
def energy_plot(energy_start, energy_end, sensitivity_energy, order):
    ...
    for group, value in value_groups.iteritems():
        df.loc[(df['Z'] != 'Treatment energy') & (df['X'] == group), 'label_pos'] += \
            float(0 if len(df.loc[(df['Z'] == 'Treatment energy') & (df['X'] == group), 'Y']) == 0 else df.loc[
                (df['Z'] == 'Treatment energy') & (df['X'] == group), 'Y'])
        df.loc[(df['Z'] == 'Desalination energy') & (df['X'] == group), 'label_pos'] += float(
            df.loc[(df['Z'] == 'Pumping energy') & (df['X'] == group), 'Y'])
        df.loc[(df['X'] == group), 'total'] = value

    sensitivity_df = pd.DataFrame()
    for scenario, item in sensitivity_energy.items():
        if scenario != 'Baseline':
            item['Desalination energy'] += df_end.loc[
                (df_end['X'] == scenario) & (df_end['Z'] != 'Desalination energy'), 'Y'].sum()
            item['Pumping energy'] += float(
                df_end.loc[(df_end['X'] == scenario) & (df_end['Z'] == 'Treatment energy'), 'Y'])
    ...
    p = (ggplot(df) + geom_bar(df, aes(x='X_cat', y='Y', fill='Z'), stat='identity', size=0.3, color='gray',
                               width=0.5) +
         geom_text(df, aes(x='X_cat', y='label_pos',
                           label='[int(round(x,0)) if (x/t)>0.05 else "" for x, t in zip(Y,total)]'), color='#e7e7e7',
                   nudge_y=0, size=8) +
         labs(y='Energy (GWh/yr)', x='Scenario') +
         scale_fill_brewer(type='qual', palette='Set2') +
         scale_color_brewer(type='qual', palette='Set2') +
         coord_flip() +
         theme_minimal() + geom_vline(xintercept=[1.5 + x for x in range(len(sensitivity_energy.items()) - 1)],
    ...
    for var in set(sensitivity_df['SensitivityVar']):
        nudge *= -1
        p = (p + geom_line(sensitivity_df.loc[sensitivity_df['SensitivityVar'] == var],
                           aes(x='Scenario', y='value', group='group', color='variable'),
                           position=position_nudge(x=nudge)) +
             geom_point(sensitivity_df.loc[sensitivity_df['SensitivityVar'] == var],
                        aes(x='Scenario', y='value', group='group', color='variable'), shape='|',
                        position=position_nudge(x=nudge), size=2))
...
def water_plot(withdrawals_total, order):
    ...
    for scenario, value in zip(order, [withdrawals_total[key] for key in
                                       sorted(withdrawals_total, key=lambda i: order.index(i))]):
        ...
        highs.loc[highs['variable'] == 'Irrigation extractions', 'label_pos'] = highs['value'].sum() - highs[
            'value'] / 2
        lows['label_pos'] = lows['value'] / 2
        lows.loc[lows['variable'] == 'Reused water from irrigation', 'label_pos'] = lows['value'].sum() - lows[
            'value'] / 2
        ...
        p = (p + geom_bar(highs, aes(x='scenario', y='value', fill='variable'), stat='identity', position='stack',
                          width=0.5, size=0.3, color='gray')
             + geom_bar(lows, aes(x='scenario', y='value', fill='variable'), stat='identity', position='stack',
                        width=0.5, size=0.3, color='gray')
             + geom_text(highs, aes(x='scenario', y='label_pos',
                                    label='[str(int(x)) if (x/t)>0.05 else "" for x, t in zip(value,total)]'), size=7,
                         color='#F7F7F7')
             + geom_text(lows, aes(x='scenario', y='label_pos',
                                   label='[str(int(x)) if (x/t)>0.05 else "" for x, t in zip(-value,total)]'), size=7,
                         color='#F7F7F7')
             + geom_line(lows, aes(x='scenario', y='[0,sum(value)/2-np.average(percentage_pos)]'),
                         position=position_nudge(x=0.4))
             + geom_line(lows, aes(x='scenario', y='[sum(value)/2+np.average(percentage_pos),sum(value)]'),
                         position=position_nudge(x=0.4))
             + geom_point(lows, aes(x='scenario', y='sum(value)'), shape='|', size=2, position=position_nudge(x=0.4))
             + geom_text(lows, aes(x='scenario', y='sum(value)/2',
                                   label='str(round(-sum(value)/np.average(total)*100,1)) + "%" if sum(value)<0 else ""'),
                         size=7, position=position_nudge(x=0.4)))
...
def water_plot_per_cluster(withdrawals_per_cluster, cluster, admin_names):
    ...
    for scenario, value in withdrawals_per_cluster.items():
        ...
        p = (p + geom_bar(highs, aes(x='cluster_cat', y='value', fill='variable'), stat='identity', position='stack',
                          width=0.5)
             + geom_bar(lows, aes(x='cluster_cat', y='value', fill='variable'), stat='identity', position='stack',
                        width=0.5))
        ...
        p = (p + theme_minimal() + theme(legend_position='top', legend_title=element_blank())
             + geom_hline(yintercept=0, color="black") + scale_fill_brewer(type='div', palette='PuOr',
                                                                           labels=['Irrigation extractions',
                                                                                   'Population extractions',
                                                                                   'Reused water from irrigation',
                                                                                   'Reused water from population'])
             + geom_text(text_df_1, aes(x='X', y='Y', label='label'), ha='right')
             + geom_text(text_df_2, aes(x='X', y='Y', label='label'), ha='left')
             + expand_limits(x=len(set(highs['Cluster'])) + 4) + labs(x='Cluster',
                                                                      y='Million cubic meters of water per year (Mm3/yr)'))
...
def tech_plot(data, pop_dic, agri_dic, order):
    ...
    p = (ggplot(df_end) + geom_bar(aes(x='type', y='value', fill='tech'), stat='identity', position='fill', width=0.8,
                                   size=0.3, color='gray') +
         geom_text(aes(x='type', y='value/total', fill='tech',
                       label='[str(int(x/t * 100)) + "%" if x/t>0.05 else "" for x, t in zip(value,total)]'), size=7,
                   color='#F7F7F7', position=position_stack(vjust=0.5)) +
